...
describe('MessagingService', () => {
  ...
  describe('驗證功能', () => {
    describe('validateToken', () => {
      ...
      test('應該拒絕空的 token', () => {
        expect(() => messagingService.validateToken('')).toThrow('設備 token 不能為空');
        expect(() => messagingService.validateToken(null)).toThrow('設備 token 不能為空');
        expect(() => messagingService.validateToken(undefined)).toThrow('設備 token 不能為空');
      });

      test('應該拒絕非字符串的 token', () => {
        expect(() => messagingService.validateToken(123)).toThrow('設備 token 不能為空');
        expect(() => messagingService.validateToken({})).toThrow('設備 token 不能為空');
      });

      test('應該拒絕過短的 token', () => {
        expect(() => messagingService.validateToken('short')).toThrow('設備 token 格式不正確');
      });
    });
    ...
    describe('validateNotification', () => {
      test('應該接受有效的通知', () => {
        ...
        expect(() => messagingService.validateNotification(validNotification)).not.toThrow();
      });

      test('應該接受只有標題的通知', () => {
        const notification = { title: '只有標題' };
        expect(() => messagingService.validateNotification(notification)).not.toThrow();
      });
      ...
      test('應該拒絕空的通知', () => {
        expect(() => messagingService.validateNotification(null)).toThrow('通知內容不能為空');
        expect(() => messagingService.validateNotification(undefined)).toThrow('通知內容不能為空');
      });

      test('應該拒絕沒有標題和內容的通知', () => {
        expect(() => messagingService.validateNotification({})).toThrow('通知必須包含標題或內容');
      });
      ...
      test('應該拒絕過長的標題', () => {
        ...
        expect(() => messagingService.validateNotification(notification)).toThrow(
          '通知標題過長，最多 200 字符',
        );
      });
  ...
  describe('發送到單一設備', () => {
    ...
    test('應該成功發送通知到單一設備', async () => {
      const result = await messagingService.sendToDevice(validToken, validNotification);
    ...
    test('應該包含完整的訊息配置', async () => {
      ...
      const sentMessage = mockSend.mock.calls[0][0];
      expect(sentMessage.notification.imageUrl).toBe('https://example.com/image.jpg');
    ...
    test('應該支援自定義選項', async () => {
      ...
      await messagingService.sendToDevice(validToken, validNotification, {}, options);
    ...
    test('發送失敗時應該拋出錯誤', async () => {
      ...
      await expect(messagingService.sendToDevice(validToken, validNotification)).rejects.toThrow(
        '通知發送失敗: 網路錯誤',
      );
    });
  ...
  describe('發送到多個設備', () => {
    ...
    test('應該成功發送通知到多個設備', async () => {
      const result = await messagingService.sendToMultipleDevices(validTokens, validNotification);
    ...
    test('應該拒絕空的 token 列表', async () => {
      await expect(messagingService.sendToMultipleDevices([], validNotification)).rejects.toThrow(
        '設備 token 列表不能為空',
      );
    });
  ...
  describe('發送到主題', () => {
    ...
    test('應該成功發送通知到主題', async () => {
      const result = await messagingService.sendToTopic('news', validNotification);
    ...
    test('應該支援條件表達式', async () => {
      ...
      await messagingService.sendToTopic('news', validNotification, {}, { condition });
    ...
    test('應該拒絕無效的主題名稱', async () => {
      await expect(messagingService.sendToTopic('', validNotification)).rejects.toThrow(
        '主題名稱不能為空',
      );
  ...
  describe('主題訂閱管理', () => {
    ...
    test('應該成功訂閱主題', async () => {
      const result = await messagingService.subscribeToTopic(validToken, 'news');
    ...
    test('應該成功取消訂閱主題', async () => {
      const result = await messagingService.unsubscribeFromTopic(validToken, 'news');

      expect(result.success).toBe(true);
      expect(result.successCount).toBe(1);
      expect(result.failureCount).toBe(0);
      expect(mockUnsubscribeFromTopic).toHaveBeenCalledWith([validToken], 'news');
    });

    test('應該拒絕無效的主題名稱', async () => {
      await expect(messagingService.subscribeToTopic(validToken, '')).rejects.toThrow(
        '主題名稱不能為空',
      );

      await expect(messagingService.unsubscribeFromTopic(validToken, '')).rejects.toThrow(
        '主題名稱不能為空',
      );
    });
  });
  ...
  describe('通知模板', () => {
    test('應該獲取預設模板', () => {
      const welcomeTemplate = messagingService.createNotificationTemplate('welcome');
      expect(welcomeTemplate.title).toBe('歡迎加入 Localite！');
      expect(welcomeTemplate.color).toBe('#2196F3');

      const tourTemplate = messagingService.createNotificationTemplate('tour_ready');
    ...
    test('應該獲取自定義模板', () => {
      ...
      const result = messagingService.createNotificationTemplate('custom', customTemplate);
      expect(result).toEqual(customTemplate);
    });

    test('應該返回預設模板當模板不存在時', () => {
      const defaultTemplate = messagingService.createNotificationTemplate('non-existent');
  ...
  describe('使用模板發送通知', () => {
    ...
    test('應該使用模板發送到單一設備', async () => {
      ...
      const result = await messagingService.sendWithTemplate(validToken, 'welcome', variables);
    ...
    test('應該使用模板發送到多個設備', async () => {
      ...
      const result = await messagingService.sendWithTemplate(tokens, 'tour_ready');
    ...
    test('應該使用模板發送到主題', async () => {
      const result = await messagingService.sendWithTemplate('/topics/news', 'reminder');
    ...
    test('應該拒絕無效的目標類型', async () => {
      await expect(messagingService.sendWithTemplate(123, 'welcome')).rejects.toThrow(
        '無效的目標類型',
      );
    });
  });
